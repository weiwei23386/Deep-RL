...
class LoadedGaussianPolicy(BasePolicy, nn.Module):
    ...
    def update(self, obs_no, acs_na, adv_n=None, acs_labels_na=None):
        ...
        """)
    # because it's from a metaclss BasePolicy, it has to define the update.

    def get_action(self, obs):
        # ensure the consistency of the dimensions
        if len(obs.shape) > 1:
            observation = obs
        else:
            observation = obs[None, :]
        # make torch from numpy
        observation = ptu.from_numpy(observation.astype(np.float32))
        action = self(observation) # pyTorch __call__
        return ptu.to_numpy(action) # give PyTorch tensor
